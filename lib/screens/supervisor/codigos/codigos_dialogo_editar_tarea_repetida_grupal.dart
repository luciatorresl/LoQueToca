...
class DialogoEditarTareaRepetidaGrupal extends StatefulWidget {
  /// Regla maestra (doc de la coleccion tareas_grupales_repetidas).
  final TareaRepetida regla;

  const DialogoEditarTareaRepetidaGrupal({super.key, required this.regla});

  @override
  State<DialogoEditarTareaRepetidaGrupal> createState() =>
      _DialogoEditarTareaRepetidaGrupalState();
}

class _DialogoEditarTareaRepetidaGrupalState
    extends State<DialogoEditarTareaRepetidaGrupal> {
  final _servicio = ServicioTareasRepetidas();
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Editar tarea repetida grupal'),
  ...
  Future<void> _guardar() async {
    ...
    if (_conFechaFin && _fechaFin == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Elige una fecha de fin o desactiva la opción')),
      );
      return;
    }
    ...
    try {
      await _servicio.editarGrupal(
        widget.regla.id, // id de la regla maestra
        titulo: _titulo.text,
        hora: horaStr,
        patron: _patron,
